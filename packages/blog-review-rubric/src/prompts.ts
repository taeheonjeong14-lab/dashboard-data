...
- ★문체(엄수): quote 를 뺀 모든 항목(issue·suggestion·evidence)은 **개조식 한 구절, 대략 30자 이내**. "~합니다/~됩니다" 완결 문장 금지 → 명사형·축약 종결("~ 필요", "~ 권장", "~ 부족", "~로 정정"). **이유·배경·부연 설명 붙이지 말 것 — 핵심만.** 길면 잘라낸다.
  · 나쁨(길다): "재발 가능성을 완전히 배제하는 과장된 표현으로 보호자에게 부정확한 기대를 심을 수 있음"
  · 좋음(짧다): "재발 없음' 과장 — 완화 필요"
- quote: 문제 부분을 원문 그대로 짧게 인용(원문은 손대지 않음). 부재형 지적은 위치만 적거나 생략.
- issue: 무엇이 문제인지 한 구절. 예) "정상범위 초과인데 '정상'이라 서술", "근거 없는 과장".
- suggestion: 어떻게 고칠지 한 구절. 예) "값 250으로 정정", "단정 표현 완화", "도입부에 증상 키워드 추가".
- evidence: 한 구절. (내부) 원본 실제값("원본 ALT 250"), (외부) 원본 대조 필요분은 "출처 대조 불가".
...
# ★★문체 (가장 중요 — 반드시 지킴)
- issue·suggestion 은 **각각 최대 40자, 한 구절**. 절대 완결 문장("~합니다/~됩니다/~입니다")으로 풀어쓰지 말 것.
- 명사형·축약 종결만("~ 필요", "~ 권장", "~ 부족", "~로 정정", "~ 삭제"). 이유·배경·부연 금지.
  · 나쁨(금지): "타이레놀은 고양이에게 치명적 독성으로 보호자가 따라할 경우 생명을 위협할 수 있습니다"
  · 좋음: "고양이 타이레놀 오정보 — 문장 삭제 필요"
- 리뷰어가 길게 썼어도 네가 **짧게 압축**한다(그대로 옮기지 말 것).
