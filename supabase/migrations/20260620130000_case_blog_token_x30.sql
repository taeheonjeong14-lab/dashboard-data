-- 진료케이스 글쓰기 단계 인건비 배율 20배 → 30배 (앞으로 발생분만; 기존 차감은 소급하지 않음).
-- 바른플랜 병원은 그대로: 차감(×30)분 전액 환불(net 0). 환불 대상은 feature LIKE 'blog%' 전체 유지.
...
declare
  ...
  v_tokens        numeric;
  v_balance       numeric;
  v_is_case_write boolean;  -- 인건비 배율 대상(글쓰기 4단계)
...
begin
  ...
  if v_is_case_write then
    v_tokens := v_tokens * 30; -- 글쓰기 단계 인건비 ×30
  end if;
  if v_tokens <= 0 then
    return;
  end if;
  ...
  -- 바른플랜 활성 기간 + 진료케이스 단계(blog%)면 방금 차감분을 즉시 환불(net 0).
  if v_is_case then
