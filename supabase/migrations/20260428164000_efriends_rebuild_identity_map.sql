-- Update rebuild_chart_for_run to resolve eFriends identities using owner+patient-set similarity.
...
begin
  ...
  if v_chart_type = 'efriends' then
    ...
    inserted as (
      ...
      select
        ...
        v_chart_type,
        m.owner_name_norm,
        encode(digest(v_hospital_id || '|efriends|' || m.owner_name_norm || '|' || gen_random_uuid()::text, 'sha256'), 'hex'),
        m.patient_names
      from missing m
  ...
  with known_links as (
    select
      ...
      case
        when v_chart_type = 'efriends'
          then encode(digest(v_hospital_id || '|efriends|visit|' || coalesce(res.customer_key_norm, r.customer_key_norm), 'sha256'), 'hex')
