-- 토큰 차감을 소수(round 2자리) → 정수 올림(ceil)으로 변경.
-- 1토큰 = $0.01. 작업(operation) 합산원가를 토큰으로 환산한 뒤 올림 → 정수 토큰 차감(최소 1).
-- 소액 올림분은 결제가에서 책정하는 마진과 별개의 미세 버림 방지(병원에 유리하지 않게)용.
-- 시그니처 동일 → create or replace 로 본문만 교체.
