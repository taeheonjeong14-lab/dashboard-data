-- Fix: rebuild_chart_for_run 의 eFriends identity 갱신 단계(merged CTE)에서
-- hospital_id / chart_type 가 chart_customer_identity_map(m) 과 불필요한
-- cross join lateral(t) 양쪽에 존재해 "column reference hospital_id is ambiguous" (42702) 발생.
-- 해당 컬럼들은 downstream UPDATE 에서 쓰이지 않으므로 select 목록과 cross join 을 제거하고,
-- 남는 컬럼은 m. 으로 명시 한정한다. 함수의 나머지 로직은 20260506201500 과 동일.
...
begin
  ...
  select min(r.service_date), max(r.service_date)
    into v_hash_min, v_hash_max
  from analytics.chart_transactions_raw r
  inner join analytics.chart_upload_runs u on u.id = p_run_id
  where r.hospital_id = v_hospital_id
    and r.chart_type = v_chart_type
    and r.source_file_hash = u.source_file_hash;

  select min(service_date), max(service_date)
    into v_run_min, v_run_max
  from analytics.chart_transactions_raw
  where run_id = p_run_id;

  if v_hash_min is null and v_run_min is null then
  ...
  if v_hash_min is null then
    v_min_date := v_run_min;
    v_max_date := v_run_max;
  elsif v_run_min is null then
    v_min_date := v_hash_min;
    v_max_date := v_hash_max;
  else
    v_min_date := least(v_hash_min, v_run_min);
    v_max_date := greatest(v_hash_max, v_run_max);
  end if;
