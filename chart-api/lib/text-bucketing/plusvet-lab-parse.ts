...
/**
 * 검사값·단위·서술에 쓰이는 "의미 있는" 문자. 라틴·숫자·한글에 값 기호(부등호·부호·괄호·마이크로)까지.
 * 이 문자가 **하나도 없는** 꼬리 토큰은 참고범위 막대 그래픽의 전사 잔여물로 본다.
 */
const MEANINGFUL_VALUE_CHARS = '0-9A-Za-z가-힣+\\-<>≤≥.,()/%#*×µμ';
/** 꼬리 그래픽 잔여 토큰(공백으로 분리된, 의미 있는 문자가 없는 토큰들). 예: "|||", "ப", "┃▮" */
const TRAILING_GRAPHIC_NOISE = new RegExp(`(?:\\s+[^\\s${MEANINGFUL_VALUE_CHARS}]+)+$`, 'u');
 ...
 *  - 꼬리 OCR 노이즈 제거: 끝의 대시런(----)·언더스코어 + 그래픽 잔여 토큰
 *
 * IDEXX 계열은 참고범위를 막대 그래프로 인쇄하고, 그게 전사되며 `-----`·`|||`·엉뚱한 글자(타밀 ப 등)로
 * 튀어나온다. 이 꼬리가 남으면 값·단위가 줄 끝에 오지 않아 **행이 통째로 드롭**된다(Osm Calc 누락).
 * 과거엔 아랍/히브리/키릴만 열거해 막았는데 타밀 ப 을 놓쳤다 → 스크립트 열거를 버리고
 * "의미 있는 문자가 없는 토큰"으로 부류 자체를 막는다. 그래야 다음 글자에 또 안 뚫린다.
 * 노이즈가 여러 토큰으로 쪼개져 나오므로(`----- ப`) 변화가 없을 때까지 반복한다.
 *
 *  - 단위 슬래시 주변 공백 제거: `ng / mL` → `ng/mL`
 *    전사가 표 셀의 구두점 양옆에 공백을 넣는 일이 있다(허브 실측: `CPL 330.3 ng / mL ↑ 50-200`,
 *    `fPL 1.2 ng / mL 0-3.5`). 단위 목록은 정확한 문자열 매칭이라 이 공백 하나에 전부 빗나가
 *    **행이 통째로 드롭**된다. 양옆이 모두 단위에 쓰이는 글자·숫자일 때만 붙이므로
 *    참고범위(`50 - 200`)나 기본정보(`축종 / 품종`, 앞이 한글)는 건드리지 않는다.
 */
function normalizeOcrArtifacts(text: string): string {
  ...
  s = s.replace(/([A-Za-z0-9μµ^*×])\s*\/\s*(?=[A-Za-z0-9μµ])/g, '$1/');
  for (let i = 0; i < 4; i += 1) {
    const before = s;
    s = s.replace(/[\s\-–—_]+$/u, ''); // 꼬리 대시런/언더스코어
    s = s.replace(TRAILING_GRAPHIC_NOISE, '');
    if (s === before) break;
  }
