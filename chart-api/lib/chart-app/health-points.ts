...
export function healthPointsPromptBlock(points: HealthPoint[], section?: string): string {
  ...
  return [
    ...
    '- ★**[「종합 소견 포함」은 게재 의무다 — 빠뜨리면 오답]** 「종합 소견 포함」 팩트를 가진 그룹은 **하나도 빠짐없이** 종합소견에 각각 번호 항목 하나로 올린다. 담당 수의사가 이미 임상적으로 판단해 체크한 것이므로, **모델이 "임상적 결론을 끌어내기 어렵다"·"경미하다"·"다른 섹션에서 다뤄진다"는 이유로 빼는 것을 금지한다.** 포인트가 수치 형태로 적혀 있으면 항목을 빼는 게 아니라 그 수치가 시사하는 결론으로 **바꿔 쓴다**.',
    '- 글자 수 상한 때문에 다 담기 어려우면 **각 항목을 더 짧게 쓴다 — 항목을 버려서 줄이지 않는다.** 종합소견에 올린 그룹은 사후관리에도 같은 번호·주제로 대응 계획이 있어야 한다.',
    '- 출력 직전에 「종합 소견 포함」 그룹 개수와 종합소견 번호 항목 개수가 맞는지 세어 확인한다.',
...
- ★**[경계값은 단독 포인트가 될 수 없다]** 참고범위를 아주 살짝 벗어난 값, 그리고 참고범위 안이지만 경계에 근접한 값(이하 "경계값")은 그 값 하나만으로는 임상적 결론이 서지 않는다. 다음 둘 중 하나로만 처리한다.
...
1) 먼저 이번 검진에서 말할 **질환·병태(임상 결론)의 목록**을 정한다. (예: "당뇨 관리 불량", "신부전 의심", "치주질환")
   **이 목록은 명확한 이상 소견으로만 세운다 — 경계값은 질환 목록을 만드는 근거가 될 수 없다.**
2) 그다음 개별 근거(문진·혈액·뇨·영상 소견)를 하나씩 보며 **그 근거가 어느 질환을 뒷받침하는지** 판단해, 그 질환을 group 으로 배정한다.
   경계값은 오직 이 단계에서, **1)에서 이미 정해진 질환에 붙을 수 있을 때만** 팩트가 된다. 붙을 곳이 없으면 버린다.

- **출처가 달라도(문진·혈액·요검사·영상) 같은 질환을 가리키면 반드시 같은 group 으로 묶는다.** 표면 표현이 달라도 임상적으로 한 질환의 근거면 하나로 모은다.
  · 예(당뇨): "당뇨 진단 후 혈당·프럭토사민 상승(BG 426, Fru 667)로 관리 미흡" + "GLU 181 상승" + "요당 1000" → group="당뇨 관리 불량". (혈액·요검사로 흩어져 있어도 전부 당뇨 조절이 안 된다는 한 이야기다.) **단, 같은 요검사의 "단백뇨 100" 은 신장 소견이므로 이 그룹이 아니라 신장 그룹(예: "신부전 의심")으로 나눠 뺀다.**
...
- inOverall: 종합 소견에 올릴 만큼 중요하면 true. **판단 기준은 "이 팩트에서 끌어낼 임상적 결론(질환·병태)이 있는가"다.**
  임상적 결론이 서는 소견이면 true, 수치가 흔들렸을 뿐 결론을 세울 수 없는 것(그룹에 보강 근거로 붙인 경계값·단독 이상·다른 근거와 안 맞음)은 false 로 둔다
  — false 여도 혈액검사 해석·장기 칸에서는 그대로 다뤄지므로 리포트에서 사라지지 않는다.
  **true 로 준 팩트는 리포트 종합 소견에 반드시 한 항목으로 실린다**(빼는 판단은 뒤 단계에서 하지 않는다). 애매하면 false 로 두고 담당자가 올린다.
