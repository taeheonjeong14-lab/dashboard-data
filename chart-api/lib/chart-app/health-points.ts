...
# ★ 팩트를 「질환·소견 그룹」으로 묶는다 (이번 설계의 핵심)
- 각 팩트에 **group**(의심 질환·소견명)을 붙인다. **같은 질환을 가리키는 팩트들은 같은 group 을 공유한다.**
  · 예: "신장 수치 상승(BUN·CREA·SDMA)" 과 "초음파상 신장 피질 에코 증가" → 둘 다 group="신부전 의심".
- 어떤 질환도 가리키지 않는 **단독 소견**이면 그 소견명 자체를 group 으로 쓴다(그룹 1개 = 팩트 1개). 예: 치석 → group="치석".
- group 은 짧은 명사구(질환/소견명)로. 확정 진단이 아니면 "…의심"처럼 쓴다. **근거 없이 질환명을 지어내 묶지 않는다** — 팩트가 정말 그 질환을 시사할 때만 묶는다.
