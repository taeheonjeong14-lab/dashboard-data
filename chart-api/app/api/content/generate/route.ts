...
- CAUSAL_FLOW의 phase 순서·인과를 따른다(진단 → 치료 순).
...
- facts 두 종류 구분:
  ① 검사·진단 → 구체적으로. 검사 수치(예: SDMA 11), 검사명(예: 복부 초음파),
     ...
     · 혈액검사 깊이: "혈액검사를 했다"(필요하면 혈구검사/혈청화학검사 등 종류까지)는 fact로 담되,
       ...
       ★정상 지표를 담을 때도 케이스 주제(주질환명)와 연관 깊은 것 위주로 몇 개만(보통 3~5개, 어떤 경우에도 10개 초과 금지).
       마취 전 안전성 평가는 예외(아래 '마취 안전성' 규칙 — 단 거기서도 10개 상한은 동일 적용).
...
- 이상 수치 + 정상·양호 수치도 facts에 포함하되, 정상 수치는 선별: ① 환자 위험요인(나이·기존질환)과 연결,
  ...
  · ★정상 수치를 나열할 때도 케이스 주제(주질환명)·마취와 연관 깊은 지표 위주로 몇 개만 고른다(보통 3~5개, 어떤 경우에도 10개 초과 금지). 정상 수치를 줄줄이 나열하지 말 것.
- 모든 수치가 정상이라 쓸 내용이 부족하면, 글의 풍부함을 위해 아래 중 하나(또는 둘 다)를 facts로 둔다:
  · 조금이라도 벗어난(off) 수치가 있으면 → "다소 높게/낮게 측정됐으나 체계적 마취로 우려할 수준은 아님" 취지.
  · off 수치도 전혀 없으면 → 환자 특성(나이·품종군·기존질환)상 유의 깊게 본 항목 몇 개만(최대 10개 이내, 보통 3~5개) + "문제없음 확인" 취지. 정상 수치를 줄줄이 나열하지 말 것.
...
- ①("혈액검사를 했다")은 반드시 언급한다. ②(종류 구분)는 케이스에 따라 써도, 안 써도 된다.
  ...
  · ★정상 지표를 언급할 때도 케이스 주제(주질환명)와 연관 깊은 것 위주로 몇 개만(보통 3~5개, 어떤 경우에도 10개 초과 금지).
