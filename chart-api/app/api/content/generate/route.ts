      ...
      "actions": [
        {
          ...
          "types": ["exam_dx | preop | surgical | postop_recovery | postop_followup | medical | admission | discharge | other 중 이 행위의 성격(반드시 1개 이상, 맞는 게 없으면 [\"other\"], 빈 배열 금지)"]
        }
      ],
...
- 구조:
  ...
  · 성격 태그(types)는 **날짜가 아니라 각 행위(action)에** 붙인다 — exam_dx(검사 및 진단: 새 질환을 진단하는 검사)/preop(술 전 검사: 수술 케이스에서 마취 가능 판단 검사)/surgical(외과: 수술 행위)/postop_recovery(술 후 회복: 수술 이후 회복·상처관리·회복기 투약)/postop_followup(술 후 경과확인: 수술 결과·회복 확인을 위한 재검사·재촬영)/medical(내과: 수술과 무관한 내과 질환 자체를 약물로 치료)/admission(입원 치료)/discharge(퇴원)/other(기타: 애매하면) 중 해당하는 것(**행위가 있으면 반드시 1개 이상**, 맞는 게 없으면 ["other"], 빈 배열 금지). ※ surgical 와 medical 는 한 행위에 동시에 넣지 말고 더 적합한 하나만(수술 행위는 surgical). 수술 후 회복·투약 행위는 medical 이 아니라 postop_recovery. 수술 후 결과·회복을 확인하는 재검사는 exam_dx 가 아니라 postop_followup.
