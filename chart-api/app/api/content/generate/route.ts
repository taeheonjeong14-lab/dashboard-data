...
- CAUSAL_FLOW(JSON): ★ 전체 흐름·순서·세부의 유일한 핵심 기준. phases 순서·인과를 따른다. 진단·치료 세부도 모두 여기서 뽑는다.
  ...
  · **이 action들의 모든 필드를 하나도 빠뜨리지 말고 해당 섹션 facts/points 로 옮긴다**(요약·생략·순서변경 최소화):
    - types 에 exam_dx·preop 가 있는 action → '진단 과정'(preop 은 '치료 과정 - 술 전 검사'). what/why/result 를 시행 순서대로 facts 에.
    - types 에 surgical 이 있는 action → '치료 과정 - 수술 당일'. **procedure 의 각 단계(step+note)를 순서 그대로 facts 에 빠짐없이 옮긴다**(수술 술기 상세).
    - types 에 postop_recovery/postop_followup 이 있는 action → '치료 과정 - 수술 후 회복'. what/result 와 재검 경과를 facts 에.
    - types 에 medical 이 있는 action → '치료 과정'(내과). what/why/result + detail(약 종류)를 facts 에.
    - types 에 aftercare 가 있는 action → '사후 관리방안'. 보호자에게 안내한 내용을 그 섹션에.
    - 각 phase 의 nextStep 은 다음 국면으로의 연결 맥락으로 활용(중복 나열 금지).
...
- CAUSAL_FLOW action 들의 types 로 치료 유형(수술/내과/복합)을 판단해 '치료 과정'을 아래처럼 구성한다. (surgical 있으면 수술, medical 위주면 내과, 둘 다면 복합)
...
- 진단 과정·치료 과정의 세부 단계·순서는 **이미 OUTLINE(인과 흐름의 action별 세부·수술 절차가 반영됨)에 담겨 있다.**
    ...
    if (contentType === BLOG_OUTLINE) {
      try {
        ...
        const userContent = [
          ...
          '진단 과정·치료 과정 섹션의 facts 에는 CAUSAL_FLOW 각 action 의 what/why/result 와, 수술 action 의 procedure 단계들(step+note), 내과 action 의 detail(약 종류)을 순서 그대로 빠짐없이 반영하세요.',
