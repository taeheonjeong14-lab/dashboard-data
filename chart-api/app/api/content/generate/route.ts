...
function normalizeActionTypes(action: unknown, caseType: string, allowAftercare = true): void {
...
function normalizeCaseTypeTags(causalFlow: unknown): void {
  ...
  phases.forEach((p, i) => {
    ...
    if (Array.isArray(actions)) for (const act of actions) normalizeActionTypes(act, ct, i === lastIdx);
  });
}
    ...
    if (contentType === BLOG_CAUSAL) {
      try {
        ...
        const { parsed: causalFlow, debug: parserDebug } = await parseJsonWithRepair(
          raw,
          'object with keys: axis (string), caseType (one of "internal"|"surgical"|"both"), anesthesia (boolean), phases (array of {id,name,period, actions:array of {what:string, why:string, result:string, types:string[], detail:string, procedure:array of {step:string, note:string}}, nextStep:string[]})',
        );
    ...
    if (contentType === BLOG_CAUSAL_PHASE) {
      try {
        ...
        if (phase && typeof phase === 'object' && Array.isArray((phase as Record<string, unknown>).actions)) {
          for (const act of (phase as Record<string, unknown>).actions as unknown[]) normalizeActionTypes(act, phaseCaseType, regenIsLast);
        }
        const debug: GenerateDebugInfo | undefined = debugEnabled
          ? { enabled: true, parser: parserDebug, model: { maxOutputTokens: stageMaxTokens } }
          : undefined;
