...
# 각 날짜의 성격(type)·마취
- 각 phase(=날짜)에 type을 붙인다: diagnostic(검사·진단 위주) / surgical(수술·마취) / medical(내과 투약·처치).
  ...
  "phases": [
    {
      ...
      "type": "diagnostic | surgical | medical",
      "actions": [
        {
...
- CAUSAL_FLOW는 **날짜별 항목**이며, 각 날짜(phase)는 actions(=행위 목록, 각 {what 무엇을, why 왜, result 결과})와 nextStep(다음 결정)으로 되어 있다. type=diagnostic 인 날들의 action → '진단 과정', surgical/medical 인 날들의 action → '치료 과정'으로 **임상 순서에 맞게 재구성**한다(날짜순 ≠ 과정순). 각 action 의 what/why/result 를 진단·치료 단계의 근거로 활용한다. 빈 날짜·주질환 무관 방문은 무시한다.
    ...
    if (contentType === BLOG_CAUSAL) {
      try {
        ...
        const { parsed: causalFlow, debug: parserDebug } = await parseJsonWithRepair(
          raw,
          'object with keys: axis (string), anesthesia (boolean), phases (array of {id,name,period,type, actions:array of {what:string, why:string, result:string}, nextStep:string[]})',
        );
        const debug: GenerateDebugInfo | undefined = debugEnabled
