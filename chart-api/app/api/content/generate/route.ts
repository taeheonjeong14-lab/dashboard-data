...
- **성격 태그는 날짜(phase)가 아니라 개별 행위(action)에 붙인다.** 각 action 에 types 배열을 넣는다(보통 1개, 여러 성격이면 여러 개 가능). 아래 기준을 정확히 따른다:
  ...
  예: 복부 초음파로 진단 → ["exam_dx"]. 마취 전 혈액검사 → ["preop"]. 종양 절제 수술 → ["surgical"]. 수술 부위 소독·회복기 투약 → ["postop_recovery"]. 수술 2주 뒤 재촬영으로 회복 확인 → ["postop_followup"]. 만성 신장질환 약물 처방 → ["medical"].
      ...
      "actions": [
        {
          ...
          "types": ["exam_dx | preop | surgical | postop_recovery | postop_followup | medical | admission | discharge | other 중 이 행위의 성격(보통 1개, 애매하면 [\"other\"], 없으면 [])"]
        }
      ],
...
- 구조:
  ...
  · 성격 태그(types)는 **날짜가 아니라 각 행위(action)에** 붙인다 — exam_dx(검사 및 진단: 새 질환을 진단하는 검사)/preop(술 전 검사: 수술 케이스에서 마취 가능 판단 검사)/surgical(외과: 수술 행위)/postop_recovery(술 후 회복: 수술 이후 회복·상처관리·회복기 투약)/postop_followup(술 후 경과확인: 수술 결과·회복 확인을 위한 재검사·재촬영)/medical(내과: 수술과 무관한 내과 질환 자체를 약물로 치료)/admission(입원 치료)/discharge(퇴원)/other(기타: 애매하면) 중 해당하는 것(보통 1개, 애매하면 ["other"], 없으면 []). ※ surgical 와 medical 는 한 행위에 동시에 넣지 말고 더 적합한 하나만(수술 행위는 surgical). 수술 후 회복·투약 행위는 medical 이 아니라 postop_recovery. 수술 후 결과·회복을 확인하는 재검사는 exam_dx 가 아니라 postop_followup.
...
# 출력 형식 — JSON only (코드펜스·다른 텍스트 없이, 딱 phase 객체 하나)
{
  ...
  "actions": [ { "what": "...", "why": "...", "result": "...", "types": ["exam_dx | preop | surgical | postop_recovery | postop_followup | medical | admission | discharge | other"] } ],
  "nextStep": ["..."]
}`;
...
- CAUSAL_FLOW는 **날짜별 항목**이며, 각 날짜(phase)는 actions(=행위 목록, 각 {what 무엇을, why 왜, result 결과, types 성격배열}), nextStep(다음 결정)으로 되어 있다. 성격(types)은 **각 행위(action)** 에 붙어 있다: exam_dx 검사및진단/preop 술전검사/surgical 외과/postop_recovery 술후회복/postop_followup 술후경과확인/medical 내과/admission 입원/discharge 퇴원/other 기타. types 에 exam_dx·preop 가 포함된 행위 → '진단 과정', surgical·postop_recovery·postop_followup·medical·admission·discharge 가 포함된 행위 → '치료 과정'으로 **임상 순서에 맞게 재구성**한다(날짜순 ≠ 과정순, 한 날짜가 양쪽에 걸칠 수 있음). 각 action 의 what/why/result 를 진단·치료 단계의 근거로 활용한다. 빈 행위·주질환 무관 방문은 무시한다.
