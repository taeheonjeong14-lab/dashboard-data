...
- surgical(수술): **procedure 의 각 단계(step+note)를 순서 그대로 facts 에 빠짐없이** 옮긴다(수술 술기 상세).
...
- 검사·수술·치료 등 진료 해시태그 섹션의 세부 단계·순서는 **이미 OUTLINE(인과 흐름의 action별 세부·수술 절차가 반영됨)에 담겨 있다.**
    ...
    if (contentType === BLOG_OUTLINE) {
      try {
        ...
        const userContent = [
          ...
          '각 해시태그 섹션의 facts 에는 그 태그가 붙은 action 의 what/why/result 와, 수술(surgical) action 의 procedure 단계들(step+note), 내과(medical) action 의 detail(약 종류)을 순서 그대로 빠짐없이 반영하세요.',
