...
- 각 날짜(phase)는 그날 시행한 "행위"들을 actions 배열로 담는다. 행위 하나 = action 하나 = { what, why, result, types, detail, procedure }.
  ...
  · detail (상세 내용): medical 행위면 처방 약 종류(문자열), 그 외엔 "". 아래 "# 상세 내용" 규칙 참고.
  · procedure (수술 절차): surgical 행위면 단계 배열 [{step, note}], 그 외엔 []. 아래 "# 상세 내용" 규칙 참고.
...
- types 에 surgical 이 있는 행위는 **procedure** 배열을 채운다. detail 은 "".
...
## medical → detail (문자열)
- types 에 medical 이 있는 행위는 detail 에 어떤 종류의 약을 처방했는지 적는다. **성분명·약효 분류("항생제","소염진통제" 등)까지만**, 제품명·정확한 용량·용법 금지. procedure 는 [].

## 그 외
- surgical·medical 이 아닌 행위는 detail 을 "" 로, procedure 를 [] 로 둔다.
      ...
      "actions": [
        {
          ...
          "detail": "medical 행위면 처방 약 종류(성분·약효 분류), 그 외엔 빈 문자열",
          "procedure": [{ "step": "surgical 행위의 절차 이름(시간순)", "note": "그 절차 부연 설명 1~2문장" }]
        }
      ],
...
- 구조:
  · actions: 행위별 { what(무엇을 했나·한 줄), why(왜·한 줄, 없으면 ""), result(결과·수치·소견 한 줄, 없으면 ""), types(이 행위의 성격 배열), detail(문자열), procedure(단계 배열) }. 시행 순서대로.
  · detail / procedure: **surgical 행위**는 procedure 를 채운다 = 수술을 시간 순서대로 나눈 단계 배열 [{step: 절차 이름, note: 그 절차 부연 설명 1~2문장}], detail="". 이 procedure 의 note 에 한해, 차트에 절차 세부가 부족하면 **수의과 교과서·신뢰할 수 있는 논문 등 표준 수의학 지식으로 더 자세히 보강**해도 된다(차트 내용 최우선, 없는 처치·환자 고유 수치 창작 금지). **medical 행위**는 detail 에 처방 약 종류(성분·약효 분류까지, 제품명·용량 금지), procedure=[]. 그 외 행위는 detail="" 이고 procedure=[].
...
# 출력 형식 — JSON only (코드펜스·다른 텍스트 없이, 딱 phase 객체 하나)
{
  ...
  "actions": [ { "what": "...", "why": "...", "result": "...", "types": ["exam_dx | preop | surgical | postop_recovery | postop_followup | medical | admission | discharge | aftercare | other"], "detail": "medical=처방 약 종류 / 그 외=빈 문자열", "procedure": [{ "step": "surgical 절차 이름(시간순)", "note": "그 절차 부연 설명" }] } ],
  "nextStep": ["..."]
}`;
