...
2. 특별한 기록이 없는 날짜도 phase는 만들되 what/why/toNext 를 빈 배열([])로 둔다. 억지로 채우지 않는다.
3. 주질환과 무관한 방문만 있는 날짜는 what 에 그날 한 것만 간단히 적고, 주질환 인과(why/toNext)엔 억지로 엮지 않는다.
  ...
  "phases": [
    {
      ...
      "what": ["그날 한 검사/처치/행위 자체 — 불릿 (결과·수치 제외). 기록 없으면 []"],
      "why": ["그것을 한 임상적 이유 — 불릿. 없으면 []"],
      "toNext": ["그날 결론(수치·진단·경과) + 다음 날짜로 이어짐 — 불릿 (마지막 날짜는 결론만, 없으면 [])"]
    }
  ]
}
(차트의 모든 방문 날짜를 빠짐없이 시간순으로. 특별한 기록이 없는 날짜도 phase 는 만들되 what/why/toNext 는 [] 로 둔다.)
...
- CAUSAL_FLOW는 이제 **날짜별 항목**이다. type=diagnostic 인 날들의 검사 → '진단 과정', surgical/medical 인 날들의 처치 → '치료 과정'으로 **임상 순서에 맞게 재구성**한다(날짜순 ≠ 과정순). 빈 날짜·주질환 무관 방문은 무시한다.
