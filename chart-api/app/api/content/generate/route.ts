...
- ★**술 전 검사·마취 전 평가(#술전검사)는 scope 를 비운다("")**. 이 검사들의 목적은 특정 질환의 진단·치료가 아니라 **마취를 견딜 수 있는 몸인지**를 보는 것이라, 주질환·동반질환 어느 쪽으로 적어도 사실과 어긋난다. 결과가 정상이든 아니든 마찬가지다.
  · ★단, **한 검사가 진단 목적을 겸하면 비우지 않는다.** 혈액검사는 "마취를 견딜 몸인지"와 "주질환이 무엇인지"를 한 번에 보는 경우가 흔하다. 그 검사가 진단 근거로도 쓰였다면 types 에 exam_dx·preop 를 **둘 다** 붙이고, scope 는 진단한 그 질환(main/comorbid)으로 둔다. scope 를 비우는 것은 **오직 마취 가능 여부 판단만이 목적인** 검사다.
  · 그 검사에서 **특정 질환이 실제로 확인되어** 그 질환을 따로 평가·처치한 행위도 그 질환의 scope 를 붙인다.
...
★표기 스타일(중요): facts 는 완결 문장("~했습니다/~하였다/~였다")으로 쓰지 말고, **수치·명사구 위주로 아주 간결하게**(전보문처럼) 적는다. 서술·해석·문장 연결은 3단계 글에서 풀어쓰므로 facts 에는 데이터만 남긴다.
  ...
  · ★★혈액검사 지표 선별(장황함 방지, 매우 중요): 이상치라도 **무조건 넣지 말 것**. 이 글의 주제(주질환·동반질환)와 직접 관련된 지표만 고른다(검사 및 진단·회복 및 경과 확인). 주제와 무관한 우연한 이상치는 facts 에서 뺀다. 보통 2~4개, 많아도 6개 안쪽.
    · 술 전 검사(preop)의 혈검은 주제 질환 관련이 아니라 **마취 안전성과 연관된 지표만**(간·신장·혈구·응고 등 마취 위험 판단 항목, 아래 '마취 안전성' 규칙). **'검사 및 진단'에 이미 넣은 지표는 여기에 다시 넣지 않는다**(아래 preop 규칙의 지표 분할).
...
- 술 전 검사(preop): 주질환·동반질환과 무관해도 "환자가 마취를 견딜 몸 상태인지" 확인하려고 본 지표·결과만.
  · 정상이든 아니든 "마취 안전성 판단"이라는 목적에 이유가 닿는 것만.
  · ★★**한 번의 혈액검사가 진단과 마취 평가 두 목적을 겸하는 경우가 흔하다.** 이때 두 섹션 **모두** 혈액검사 내용이 들어가되, **지표를 겹치지 않게 나눈다**:
    · 주질환·동반질환과 관련된 지표 → '검사 및 진단(exam_dx)' 섹션에만.
    · 그 외에 마취 위험 요소가 될 수 있는 지표(간·신장·혈구·응고·전해질 등) → '술 전 검사(preop)' 섹션에만.
    · **같은 지표를 두 섹션에 중복해 넣지 않는다.** 주제 질환 관련이면서 마취 위험이기도 한 지표(예: 담낭 케이스의 간 수치)는 '검사 및 진단'에 두고, 술 전 검사 섹션에서는 **수치를 다시 적지 않고** 그 결과를 마취 관점에서 어떻게 반영했는지만 한 줄로 잇는다(예: "앞서 확인된 간 수치 상승을 감안해 마취제·수액 계획 조정").
    · 검사는 **한 번 시행된 것**이다 — 두 번 채혈한 것처럼 읽히게 쓰지 않는다.
    · 마취 위험 지표가 실제로 하나도 없으면 이 섹션의 혈액검사 그룹은 넣지 않는다(억지로 채우지 말 것). 반대로 그런 지표가 있는데 이 섹션을 비워두는 것도 안 된다.
...
# 마취 안전성 (해당 시)
- ★**'검사 및 진단' 문단에 이미 쓴 수치를 여기서 다시 적지 않는다.** 한 번의 혈액검사로 진단과 마취 평가를 겸한 경우, 주질환·동반질환 관련 수치는 진단 문단에서 다루고, 이 문단에는 **그 외에 마취 위험이 될 수 있는 지표**를 쓴다. 진단 문단에서 다룬 수치가 마취에도 영향을 줬다면 수치를 반복하지 말고 "앞서 확인된 ~를 감안해 마취제·수액 계획을 조정했다"처럼 **관점만 이어 붙인다.** 같은 날 한 번 뽑은 피를 두 번 뽑은 것처럼 쓰지 않는다.
