...
# 섹션 구조 — "해시태그 = 섹션" (★핵심)
섹션은 두 종류다. 각 섹션은 { tag, label, points, facts, period, imageFileNames } 로 표기한다.
1) 고정 서술 섹션 (tag=""): 인과 흐름 카드가 배정되지 않는다. AI가 핵심요약(points)과 facts 를 직접 쓴다.
   - 항상 포함: 앞쪽 [인트로 · 질환 소개 · 내원 배경], 뒤쪽 [원장님 한마디 · 아웃트로]. (label 은 그 이름 그대로, tag="")
2) 해시태그 섹션 (tag=ACTION_TYPE 키): CAUSAL_FLOW action 들의 types 를 모아, **실제로 존재하는 태그마다 섹션 1개씩** 만든다.
   - 그 태그가 붙은 action 이 하나도 없으면 그 섹션은 만들지 않는다(빈 섹션 금지).
   - 한 action 에 태그가 여러 개면(예: [surgical, admission]) 그 카드는 **각 태그 섹션에 모두 등장**한다(중복 허용). 각 섹션은 "그 관점"에서 그 카드를 다룬다.
   - label = 그 태그의 한글명, tag = 그 키.

## 섹션 순서
인트로 → 질환 소개 → 내원 배경 → [존재하는 해시태그 섹션들 · 아래 ACTION_TYPE_ORDER 순서] → 원장님 한마디 → 아웃트로.

## 해시태그 종류·라벨·순서 (ACTION_TYPE_ORDER · 존재하는 것만, 이 순서대로)
1. exam_dx = "검사 및 진단"   2. preop = "술 전 검사"   3. surgical = "수술"
4. postop_recovery = "술 후 회복"   5. postop_followup = "술 후 경과확인"   6. medical = "내과 치료"
7. admission = "입원 치료"   8. discharge = "퇴원"   9. aftercare = "사후관리 안내"   10. other = "기타"
...
# 출력 형식 — JSON only
{
  ...
  "sections": [
    {
      "id": "sec_1",
      "tag": "해시태그 섹션이면 ACTION_TYPE 키(exam_dx·preop·surgical·postop_recovery·postop_followup·medical·admission·discharge·aftercare·other), 고정 서술 섹션(인트로·질환 소개·내원 배경·원장님 한마디·아웃트로)이면 \"\"",
      "label": "섹션명 (고정 섹션은 그 이름, 해시태그 섹션은 태그 한글명)",
...
- OUTLINE의 각 섹션은 { tag, label, points, facts } 를 가진다. 섹션은 두 종류다.
  · 고정 서술 섹션(tag=""): 인트로 · 질환 소개 · 내원 배경 · 원장님 한마디 · 아웃트로.
  · 해시태그 섹션(tag=키): 검사 및 진단(exam_dx) · 술 전 검사(preop) · 수술(surgical) · 술 후 회복(postop_recovery) · 술 후 경과확인(postop_followup) · 내과 치료(medical) · 입원 치료(admission) · 퇴원(discharge) · 사후관리 안내(aftercare) · 기타(other).
    ...
    if (contentType === BLOG_OUTLINE) {
      try {
        ...
        const userContent = [
          ...
          '고정 서술 섹션(인트로·질환 소개·내원 배경·원장님 한마디·아웃트로, tag="")과, CAUSAL_FLOW action 의 types 에 실제로 존재하는 태그마다의 해시태그 섹션(tag=키)을 ACTION_TYPE_ORDER 순서로 배치하세요.',
