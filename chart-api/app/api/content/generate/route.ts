...
# 각 날짜의 성격(types)·마취
- 각 phase(=날짜)에 types 배열을 붙인다. 그날의 성격에 해당하는 값을 **여러 개 넣을 수 있다**(하나만 넣어도 됨). 아래 기준을 정확히 따른다:
  · exam_dx = 검사 및 진단 : 어떤 질환인지 판단하기 위한 검사 과정이 있었던 날이면 **반드시 포함**.
  · preop = 술 전 검사 : 수술이 포함된 케이스에서, (최종 진단명과 무관할 수 있어도) 마취가 가능한지 판단하기 위한 검사를 한 날이면 **반드시 포함**.
  · surgical = 외과 치료 : 수술이 실제로 일어난 날에만 포함.
  · medical = 내과 치료 : 약을 처방하여 내과 질환을 치료하는 과정이 일어난 날에만 포함.
  · admission = 입원 치료 : (외과이건 내과이건) 병원에서 입원 치료를 받은 날이면 포함.
  · discharge = 퇴원 : 입원 치료가 포함된 케이스에서 퇴원한 날이면 포함.
  ※ surgical(외과)와 medical(내과)는 **한 날짜에 동시에 넣지 않는다. 둘 중 더 적합한 하나만** 고른다.
    수술이 일어난 날은 수술 후 투약·수액이 있어도 surgical 로만 표기(medical 금지). 수술이 없고 약물 치료만이면 medical.
  예: 검사해서 진단 내린 날 → ["exam_dx"]. 술 전 검사 후 수술하고 입원한 날 → ["preop","surgical","admission"].
- 해당 성격이 없으면(기록 없는 방문 등) types 는 [] 로 둔다.
- 전신마취를 동반한 날이 하나라도 있으면 anesthesia=true.
...
- 각 날짜(phase)는 그날 시행한 "행위"들을 actions 배열로 담는다. 행위 하나 = action 하나 = { what, why, result }.
...
- nextStep (다음 단계): 그날 결과들을 종합해 "다음에 무엇을 하기로 결정했는지". 짧은 불릿 배열. (다음 단계가 없거나 마지막 날짜면 [])
  ...
  "phases": [
    {
      ...
      "types": ["exam_dx | preop | surgical | medical | admission | discharge 중 해당하는 것들(여러 개 가능, 없으면 [])"],
      "actions": [
        {
...
- 구조:
  · types: 그날 성격 배열 — exam_dx(검사 및 진단: 질환 판단 검사 있던 날)/preop(술 전 검사: 수술 케이스에서 마취 가능 판단 검사한 날)/surgical(외과: 수술한 날)/medical(내과: 약 처방해 내과 치료한 날)/admission(입원 치료 받은 날)/discharge(입원 케이스에서 퇴원한 날) 중 해당하는 것 여러 개 가능(없으면 []). ※ surgical 와 medical 는 한 날짜에 동시에 넣지 말고 더 적합한 하나만(수술한 날은 투약이 있어도 surgical).
  · actions: 행위별 { what(무엇을 했나·한 줄), why(왜·한 줄, 없으면 ""), result(결과·수치·소견 한 줄, 없으면 "") }. 시행 순서대로.
...
- CAUSAL_FLOW는 **날짜별 항목**이며, 각 날짜(phase)는 types(성격 배열: exam_dx 검사및진단/preop 술전검사/surgical 외과/medical 내과/admission 입원/discharge 퇴원)와 actions(=행위 목록, 각 {what 무엇을, why 왜, result 결과}), nextStep(다음 결정)으로 되어 있다. types 에 exam_dx·preop 가 포함된 날의 action → '진단 과정', surgical·medical·admission·discharge 가 포함된 날의 action → '치료 과정'으로 **임상 순서에 맞게 재구성**한다(날짜순 ≠ 과정순, 한 날짜가 양쪽에 걸칠 수 있음). 각 action 의 what/why/result 를 진단·치료 단계의 근거로 활용한다. 빈 날짜·주질환 무관 방문은 무시한다.
    ...
    if (contentType === BLOG_CAUSAL) {
      try {
        ...
        const { parsed: causalFlow, debug: parserDebug } = await parseJsonWithRepair(
          raw,
          'object with keys: axis (string), anesthesia (boolean), phases (array of {id,name,period,types:string[], actions:array of {what:string, why:string, result:string}, nextStep:string[]})',
        );
        const debug: GenerateDebugInfo | undefined = debugEnabled
    ...
    if (contentType === BLOG_CAUSAL_PHASE) {
      try {
        ...
        const { parsed: phase, debug: parserDebug } = await parseJsonWithRepair(
          raw,
          'object with keys: id (string), name (string), period (string), types (string[]), actions (array of {what:string, why:string, result:string}), nextStep (string[])',
        );
        const debug: GenerateDebugInfo | undefined = debugEnabled
          ? { enabled: true, parser: parserDebug, model: { maxOutputTokens: stageMaxTokens } }
          : undefined;
