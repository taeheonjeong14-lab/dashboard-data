...
- **성격 태그는 날짜(phase)가 아니라 개별 행위(action)에 붙인다.** 각 action 에 types 배열을 넣는다(보통 1개, 여러 성격이면 여러 개 가능). 아래 기준을 정확히 따른다:
  ...
  · postop_recovery = 술 후 회복 : 수술 이후 회복을 위한 처치(통증·염증 관리, 상처·봉합 관리, 회복기 투약·수액 등).
    ...
    **수술 후 처방한 경구약(진통제·소염제·항생제 등 회복·염증 관리 목적)도 medical 이 아니라 여기(postop_recovery).**
  ...
  · medical = 내과 치료 : **수술과 무관한 내과 질환 그 자체를 약물로 치료**하는 행위(예: 만성 신장질환·심장질환·내분비 질환 등을 약으로 관리).
    수술 케이스에서 술 후에 처방하는 경구약·투약은 회복 목적이므로 medical 이 아니라 postop_recovery 로 분류한다(내과 치료로 간주하지 말 것).
  ...
  예: 복부 초음파로 진단 → ["exam_dx"]. 마취 전 혈액검사 → ["preop"]. 종양 절제 수술 → ["surgical"]. 수술 후 소독·경구 진통소염제/항생제 처방 → ["postop_recovery"]. 수술 2주 뒤 재촬영으로 회복 확인 → ["postop_followup"]. (수술과 무관한) 만성 신장질환 약물 처방 → ["medical"]. "재발 방지 위해 저지방 식이·체중 관리하세요" 안내 → ["aftercare"].
...
# 출력 형식 — JSON only (코드펜스·다른 텍스트 없이, 딱 phase 객체 하나)
{
  ...
  "actions": [ { "what": "...", "why": "...", "result": "...", "types": ["exam_dx | preop | surgical | postop_recovery | postop_followup | medical | admission | discharge | aftercare | other"] } ],
  "nextStep": ["..."]
}
...
- 진료 태그(존재할 때만 섹션으로): exam_dx · preop · surgical · postop_recovery · postop_followup · medical · admission · discharge · aftercare · other.
  · CAUSAL_FLOW action 들의 types 를 모아 **실제로 존재하는 태그마다 섹션 1개씩** 만든다. 그 태그가 붙은 action 이 없으면 그 섹션은 만들지 않는다(빈 섹션 금지).
  · 한 action 에 태그가 여러 개면(예: [surgical, admission]) 그 카드는 **각 태그 섹션에 모두 등장**한다(중복 허용). 각 섹션은 "그 관점"에서 그 카드를 다룬다.
...
7. postop_recovery = "술 후 회복"   8. postop_followup = "술 후 경과확인"   9. medical = "내과 치료"
10. admission = "입원 치료"   11. discharge = "퇴원"   12. aftercare = "사후관리 안내"   13. other = "기타"
14. director_note = "원장님 한마디"   15. outro = "아웃트로"
...
- postop_recovery(술 후 회복): 마취가 깬 직후부터 병원이 치료 종료로 판단할 때까지의 병원 내 회복 경과.
- postop_followup(술 후 경과확인): 퇴원 후 재검·경과확인 시점의 소견·수치 변화.
- medical(내과 치료): detail(약 종류)을 성분·약효 분류 수준으로 facts 에. 치료 기간의 주요 수치 변화(OO→OO)로 호전 과정을 시점 순으로(상태가 좋아지는 과정을 보여주는 게 핵심).
- aftercare(사후관리 안내): 아래 '# aftercare(사후관리 안내) 섹션' 규칙.
- admission(입원 치료)/discharge(퇴원)/other(기타): 해당 카드의 what/why/result 를 그 섹션에. other 는 다른 섹션에 억지로 끼워넣지 말 것.
...
# aftercare(사후관리 안내) 섹션 목적·어조
- 병원에서의 치료(수술이면 '술 후 회복'까지)가 끝난 뒤, 집에서 보호자가 아이를 어떻게 관리해야
  재발을 막고 건강하게 돌볼 수 있는지 원장이 보호자에게 알려주고 당부한 "집에서의 관리법"을 전하는 섹션이다.
- 병원 내 처치·수술·회복(다른 해시태그 섹션)과 명확히 구분한다 — 여기는 치료 종료 이후의 가정 관리만 담는다.
...
- 특히 헷갈리기 쉬운 경계:
  ...
  · postop_recovery(병원 내 회복) ↔ aftercare(집에서의 관리): 회복 경과는 회복 섹션에만, 가정 관리·재발 예방 당부는 사후관리 섹션에만.
...
# 출력 형식 — JSON only
{
  ...
  "sections": [
    {
      "id": "sec_1",
      "tag": "이 섹션의 ACTION_TYPE 키(모든 섹션이 태그를 가진다). 서술 섹션: 인트로=intro·질환 소개=disease_intro·내원 배경=visit_background·원장님 한마디=director_note·아웃트로=outro. 진료 섹션: exam_dx·preop·surgical·postop_recovery·postop_followup·medical·admission·discharge·aftercare·other",
...
- OUTLINE의 각 섹션은 { tag, label, points, facts } 를 가진다. 모든 섹션이 태그를 가지며, 성격은 두 갈래다.
  ...
  · 진료 섹션: 검사 및 진단(exam_dx) · 술 전 검사(preop) · 수술(surgical) · 술 후 회복(postop_recovery) · 술 후 경과확인(postop_followup) · 내과 치료(medical) · 입원 치료(admission) · 퇴원(discharge) · 사후관리 안내(aftercare) · 기타(other).
- 아래 규칙에서 옛 섹션명이 나오면 이 해시태그 섹션으로 대응해 읽는다: '진단 과정'≈검사 및 진단, '치료 과정'≈수술/술 후 회복/내과 치료 등 진료 해시태그 섹션들, '사후 관리방안'≈사후관리 안내.
...
- 그 외 섹션(질환 소개, 검사 및 진단·수술·내과 치료 등 진료 해시태그 섹션, 사후관리 안내 등)에서는 병원 이름을 쓰지 않는다.
...
- 섹션 전환(transition): 아래 경계에서만 각 섹션 끝에 다음으로 자연스럽게 넘어가는 짧은 전환 문장(1~2문장)을 둔다. (그 외 섹션엔 굳이 안 넣어도 됨)
  ...
  · 진료(치료) 해시태그 섹션이 여럿(예: 술 전 검사 → 수술 → 술 후 회복)인 경우, 그 섹션들 사이
  · 마지막 진료(치료) 해시태그 섹션 → 사후관리 안내
- 치료 마무리 → 사후관리 안내(중요): 병원 내 치료(마지막 진료 해시태그 섹션)의 끝은 "치료가 잘 마무리되었다"를 보여주며 사후관리 안내로 넘어간다(마케팅상 중요).
