...
# 각 날짜의 성격(types)·마취
- 각 phase(=날짜)에 types 배열을 붙인다. 그날의 성격에 해당하는 값을 **여러 개 넣을 수 있다**(하나만 넣어도 됨). 가능한 값:
  · diagnostic = 검사 (혈액·영상·현미경 등 검사 시행)
  · diagnosis = 진단 (검사 결과로 질환을 확정·판단)
  · preop = 술 전 검사 (마취·수술 전 안전성 평가)
  · surgical = 외과 치료 (수술·마취 동반 처치)
  · medical = 내과 치료 (투약·주사·내과적 처치)
  예: 검사하고 그 자리에서 진단까지 내린 날 → ["diagnostic","diagnosis"]. 술 전 검사 후 수술한 날 → ["preop","surgical"].
  ...
  "phases": [
    {
      ...
      "types": ["diagnostic | diagnosis | preop | surgical | medical 중 해당하는 것들(여러 개 가능, 없으면 [])"],
      "actions": [
        {
...
- 구조:
  · types: 그날 성격 배열 — diagnostic(검사)/diagnosis(진단)/preop(술 전 검사)/surgical(외과)/medical(내과) 중 해당하는 것 여러 개 가능(없으면 []).
...
- CAUSAL_FLOW는 **날짜별 항목**이며, 각 날짜(phase)는 types(성격 배열: diagnostic 검사/diagnosis 진단/preop 술전검사/surgical 외과/medical 내과)와 actions(=행위 목록, 각 {what 무엇을, why 왜, result 결과}), nextStep(다음 결정)으로 되어 있다. types 에 diagnostic·diagnosis·preop 가 포함된 날의 action → '진단 과정', surgical·medical 가 포함된 날의 action → '치료 과정'으로 **임상 순서에 맞게 재구성**한다(날짜순 ≠ 과정순, 한 날짜가 양쪽에 걸칠 수 있음). 각 action 의 what/why/result 를 진단·치료 단계의 근거로 활용한다. 빈 날짜·주질환 무관 방문은 무시한다.
