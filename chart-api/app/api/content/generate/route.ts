    ...
    if (contentType === BLOG_CAUSAL) {
      try {
        ...
        const { parsed: causalFlow, debug: parserDebug } = await parseJsonWithRepair(
          raw,
          'object with keys: axis (string), caseType (one of "internal"|"surgical"|"both"), chronicManagement (boolean), anesthesia (boolean), phases (array of {id,name,period, actions:array of {what:string, why:string, result:string, types:string[], scope:string, detail:string, procedure:array of {step:string, note:string}}, nextStep:string[]})',
        );
    ...
    if (contentType === BLOG_CAUSAL_PHASE) {
      try {
        ...
        const { parsed: phase, debug: parserDebug } = await parseJsonWithRepair(
          raw,
          'object with keys: id (string), name (string), period (string), actions (array of {what:string, why:string, result:string, types:string[], scope:string, detail:string, procedure:array of {step:string, note:string}}), nextStep (string[])',
        );
