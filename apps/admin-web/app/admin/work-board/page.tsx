...
export default function WorkBoardPage() {
  ...
  const colHeader: React.CSSProperties = {
    display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10, fontSize: 14, fontWeight: 700, color: 'var(--text)',
  };

  return (
    <div style={{ paddingBottom: 40 }}>
      <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', margin: '0 0 4px' }}>작업 현황</h1>
      <p style={{ fontSize: 14, color: 'var(--text-secondary)', margin: '0 0 18px' }}>병원이 요청한 검진리포트·진료케이스(블로그) 작업의 현황과 잔여 목록입니다.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20, alignItems: 'flex-start' }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={colHeader}>
              ...
              <span>병원별 잔여 수량</span>
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-muted)' }}>{pending.length}건</span>
            </div>
            {pendingSummary.length > 0 ? (
              ...
              <p style={{ fontSize: 14, color: 'var(--text-muted)', padding: '40px 0', textAlign: 'center' }}>잔여 작업이 없습니다. 👍</p>
            )}
          </div>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={colHeader}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-muted)' }}>{recentHealthDone.length}건</span>
            </div>
            {recentHealthDone.length === 0 ? (
              <p style={{ fontSize: 14, color: 'var(--text-muted)', padding: '24px 0', textAlign: 'center' }}>최근 1주일간 완료된 검진 리포트가 없습니다.</p>
            ) : (
          ...
          <div style={{ minWidth: 0 }}>
            <div style={colHeader}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-muted)' }}>{recentDrafted.length}건</span>
            </div>
            {recentDrafted.length === 0 ? (
              <p style={{ fontSize: 14, color: 'var(--text-muted)', padding: '24px 0', textAlign: 'center' }}>최근 1주일간 작성완료된 블로그가 없습니다.</p>
            ) : (
              <div style={{ display: 'grid', gap: 8 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={colHeader}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-muted)' }}>{recentSaved.length}건</span>
            </div>
            {recentSaved.length === 0 ? (
              <p style={{ fontSize: 14, color: 'var(--text-muted)', padding: '24px 0', textAlign: 'center' }}>최근 1주일간 저장완료된 블로그가 없습니다.</p>
            ) : (
              <div style={{ display: 'grid', gap: 8 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            <select value={hospital} onChange={(e) => setHospital(e.target.value)}
              style={{ padding: '6px 10px', fontSize: 14, borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)' }}>
              <option value="">전체 병원</option>
              {hospitals.map((h) => <option key={h} value={h}>{h}</option>)}
            </select>
            {hospital && (
              <button type="button" onClick={() => setHospital('')}
                style={{ padding: '6px 10px', fontSize: 14, fontWeight: 500, borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                필터 해제 ✕
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 320 }}>
              <div style={colHeader}>
                ...
                <span>건강검진 리포트</span>
                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-muted)' }}>{pendingHealth.length}건</span>
              </div>
              {pendingHealth.length === 0 ? (
                <p style={{ fontSize: 14, color: 'var(--text-muted)', padding: '30px 0', textAlign: 'center' }}>잔여 검진 리포트가 없습니다.</p>
              ) : (
                <div style={{ display: 'grid', gap: 8 }}>
            ...
            <div style={{ flex: 1, minWidth: 320 }}>
              <div style={colHeader}>
                <span style={{ display: 'inline-flex', width: 4, height: 14, borderRadius: 2, background: '#dc2626' }} />
                <span>블로그</span>
                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-muted)' }}>{pendingBlog.length}건</span>
              </div>
              {pendingBlog.length === 0 ? (
                <p style={{ fontSize: 14, color: 'var(--text-muted)', padding: '30px 0', textAlign: 'center' }}>잔여 블로그가 없습니다.</p>
              ) : (
                <div style={{ display: 'grid', gap: 8 }}>
