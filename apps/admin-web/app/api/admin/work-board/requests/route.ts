...
type Row = {
  ...
  requester: string | null; due_date: string | null;
  sort_order: number; created_at: string;
};
function toDto(r: Row) {
  return {
    id: r.id, runId: r.run_id, board: r.board,
    requester: r.requester ?? '', dueDate: r.due_date ?? null,
...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await sb
      ...
      .from('work_requests')
      .select('id, run_id, board, requester, due_date, sort_order, created_at')
...
export async function POST(request: NextRequest) {
  ...
  let body: { runId?: string; runIds?: unknown; board?: string; requester?: string; dueDate?: string | null };
  try {
  ...
  try {
    ...
    const rows = fresh.map((id, i) => ({
      run_id: id, board, requester, due_date: dueDate, sort_order: i, created_at: createdAt, created_by: gate.userId,
    }));
    const { data, error } = await sb
      .schema('health_report')
      .from('work_requests')
