...
// 질환 소개 박스 본문 온디맨드 생성 — chart-api로 프록시.
export async function POST(request: NextRequest) {
  ...
  if (!cfg) {
    return NextResponse.json({ error: 'Chart API 프록시가 설정되지 않았습니다.' }, { status: 503 });
  }
  ...
  try {
    const res = await fetch(`${cfg.outboundBase}/api/content/health-checkup/disease-intro`, {
      method: 'POST',
      headers: {
