...
/** chart-api `/api/content/blog/draft-diff/list` 프록시 — 프롬프트 개선 > 블로그 컨텐츠 목록. */
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(
      `${cfg.outboundBase}/api/content/blog/draft-diff/list?limit=${encodeURIComponent(limit)}`,
...
/**
 * 실패한 분석 재시도 프록시 — chart-api `/api/content/blog/draft-diff` 에 action:'retry'.
