...
// 블로그: 요청(작성 전) → 작성중(작성 시작~확정 전) → 작성완료(확정) → 저장완료(네이버 임시저장 확인)
// 검진: 요청(리포트 생성 전) → 완료(생성 후)
export type BlogStage = 'none' | 'requested' | 'writing' | 'drafted' | 'saved';
export type HealthStage = 'none' | 'requested' | 'done';

const BLOG_GEN_TYPES = ['blog_causal', 'blog_detail', 'blog_outline', 'blog_post'];

/**
 * types = 그 run 의 generated_run_content content_type 집합
 * blogConfirmed = blog_post.payload.confirmed (4단계 작성 확정)
 * blogSaved = blog_post.payload.saved (네이버 임시저장 완료)
 */
export function computeBlogStage(types: Set<string>, blogConfirmed: boolean, blogSaved = false): BlogStage {
  if (!types.has('blog_case')) return 'none';
  if (blogConfirmed && blogSaved) return 'saved';
  if (blogConfirmed) return 'drafted';
...
// 배지 색 규칙 (모든 화면 공통):
//  배경 = 카테고리 (블로그 노랑 / 검진리포트 파랑), 글자색은 단계 '단어'에만 입힌다.
//  요청=빨강, 작성중=주황, 작성완료=초록, 저장완료/완료=파랑. 카테고리 단어(블로그/검진리포트)는 기본색.
export type BadgeCategory = 'blog' | 'health';
export type StageKey = 'requested' | 'writing' | 'drafted' | 'saved' | 'done';
...
export const STAGE_WORD: Record<StageKey, string> = {
  requested: '요청', writing: '작성중', drafted: '작성완료', saved: '저장완료', done: '완료',
};
...
// 필터용 — 타입(블로그/검진리포트), 단계(요청/작성중/작성완료/저장완료/완료)
export const TYPE_FILTERS = ['블로그', '검진리포트'] as const;
export const STAGE_FILTERS = ['요청', '작성중', '작성완료', '저장완료', '완료'] as const;
...
const BLOG_STAGE_TO_FILTER: Record<Exclude<BlogStage, 'none'>, StageFilter> = {
  requested: '요청', writing: '작성중', drafted: '작성완료', saved: '저장완료',
};
