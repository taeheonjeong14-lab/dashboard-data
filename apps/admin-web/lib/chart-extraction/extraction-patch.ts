...
import { chartByDateIdForDateTime, getParseRun } from '@/lib/chart-extraction/run-queries';
...
async function patchVaccination(client: pg.PoolClient, runId: string, records: unknown[]) {
  ...
  for (const r of records) {
    ...
    const administered_date = row.administeredDate !== undefined ? str(row.administeredDate) : null;
...
async function patchPlan(client: pg.PoolClient, runId: string, body: Record<string, unknown>) {
  ...
  for (const pr of rows) {
    ...
    if (!chartByDateId) throw new Error(`no result_chart_by_date for dateTime=${dateTime}`);
