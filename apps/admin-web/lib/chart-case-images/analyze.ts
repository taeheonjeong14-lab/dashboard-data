...
async function runImageGroupRequest(
  ...
  const prompt = [
    ...
    '  · **장비를 단정할 시각적 근거가 없으면 other 를 쓴다.** 틀린 장비명이 붙는 것보다 부위만 남기는 편이 낫다.',
    '  · slit_lamp(세극등)은 특수 안과 장비다 — 가늘고 밝은 띠 모양 광선, 청색광/형광 염색, 각막 단면 조명 같은 **그 장비 특유의 근거가 보일 때만** 쓴다.',
    '    일반 카메라로 찍은 눈 사진은 slit_lamp 가 아니라 other + bodyPart "우안"/"좌안" 이다.',
    '  · endoscopy(검이경)도 같다 — 검이경 특유의 원통형 시야가 아니면 other + bodyPart(예: "좌측 외이도").',
    '- bodyPart: 검사 부위를 한국어로 짧게 (예: 흉부, 복부, 좌측 무릎, 구강, 우안). 불명확하면 "".',
    '  · **흉부·복부 방사선은 촬영 방향을 함께 적는다** — "흉부 정면"/"흉부 측면"/"복부 정면"/"복부 측면".',
    '    정면 = VD(복배)·DV(배복), 측면 = lateral. 리포트가 이 순서로 이미지를 배치하므로 방향이 중요하다.',
    '    방향을 알 수 없으면 방향 없이 부위만 적는다(추측 금지).',
