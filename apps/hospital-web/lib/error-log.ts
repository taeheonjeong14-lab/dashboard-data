...
import { capSize, redactPayload } from '@/lib/redact';

/**
 * core.error_logs 적재. admin 에러 로그 화면(/admin/error-logs)의 원본.
 * 마스킹 규칙은 lib/redact.ts 한 곳에만 있다.
 */
...
export async function logError(input: ErrorLogInput): Promise<void> {
  try {
    ...
    console.error('[error-log] 적재 실패:', e);
  }
}
