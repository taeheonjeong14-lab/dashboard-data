...
export function consentRequiredText(hospitalName: string): string {
  return [
    ...
    `■ 수집·이용 목적: 진단, 치료 및 입원, 진료 및 검사 예약, 예약 조회 및 일정 고지, 원무 서비스`,
    `■ 수집 항목: 보호자 성명·연락처·주소, 반려동물 정보(이름·종/품종·성별·생년월일 또는 나이), 진료 관련 정보(증상·병력·예방접종·생활 정보 등)`,
    `■ 보유·이용 기간: 서비스 제공 종료 혹은 동의 철회 시까지`,
    `■ 처리 위탁 — 수탁업체: ${PROCESSOR} / 업무: 사전문진·진료정보의 저장·분석 및 AI 기반 진료 보조 처리`,
