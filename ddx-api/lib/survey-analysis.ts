...
- **[경과 확인(재진) 방문일 때만 적용]** meta.visitType 이 "경과 확인" 또는 "재진"이면, 이 블록 안을 아래 두 개의 소제목으로 나눠서 작성한다. 두 소제목 역시 양옆에 별표 두 개를 붙여 굵게 표기한다(예: **지난 진료 내용**).
  **지난 진료 내용**
  - meta.previousChartText(이전 진료 차트)를 바탕으로, 지난 진료의 주호소·진단(또는 의심질환)·시행한 처치·처방을 임상의 시선으로 간결히 정리한다. ("- " 들머리 사용)
  **지난 진료 이후 내용**
  - 이번 사전문진 응답을 바탕으로, 지난 진료 이후의 경과(처방약 반응, 증상 호전·유지·악화, 새로 생긴 증상, 식욕·활력 변화 등)를 위의 들머리 규칙대로 정리한다.
  신규환자·새 증상 방문(경과 확인이 아닌 경우)이면 이 두 소제목을 쓰지 말고 위 기본 규칙대로만 작성한다.
