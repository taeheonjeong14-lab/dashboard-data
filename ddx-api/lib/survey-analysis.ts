...
import { isFollowUpVisit } from '@/lib/survey-questions';
...
- (단, 경과 확인 방문이면 이 블록을 별도 규칙으로 작성한다 — 입력 끝에 따로 지시가 붙는다.)
...
요약 규칙:
- **[가장 중요] 요약에는 사전문진 응답(경과 확인 방문의 "지난 진료 내용" 소제목은 meta.previousChartText)에 실제로 존재하는 내용만 쓴다. 그 외의 정보는 단 한 글자도 지어내거나 추론·추가하지 않는다.** 증상·수치·병명·기간 등 어떤 항목도 근거 자료에 없으면 적지 않는다. 추정/짐작 표현 금지. (이 환각 금지 규칙은 요약에만 적용된다 — DDx·추가 질문은 임상 추론을 해도 된다.)
...
// 경과 확인(재진) 방문 전용 — "주된 호소 및 경과" 블록을 두 소제목으로 강제 분리한다.
// 조건부가 아니라 "무조건 이렇게" 지시라 모델이 기본 예시를 따라가지 않고 분리 형식을 지킨다.
const FOLLOW_UP_SUMMARY_RULE = `## ★★ 이번 건은 "경과 확인(재진)" 방문이다 — 아래를 반드시 그대로 적용 ★★
"주된 호소 및 경과" 블록 안은 위 기본 형식 대신, 반드시 아래 두 소제목으로 나눠서 작성한다.
두 소제목은 대괄호로 감싸 [지난 진료 내용], [지난 진료 이후] 형태로 각각 한 줄에 적는다(별표는 쓰지 않는다).

[지난 진료 내용]
- meta.previousChartText(이전 진료 차트)를 근거로 지난 진료의 주호소·진단(또는 의심질환)·시행한 처치·처방을 임상의 시선으로 간결히 정리한다. ("- " 들머리)
- previousChartText 가 비어 있으면 이 소제목 줄은 생략한다.

[지난 진료 이후]
- 이번 사전문진 응답을 근거로 지난 진료 이후의 경과(처방약 반응, 증상 호전·유지·악화, 새로 생긴 증상, 식욕·활력 변화 등)를 "- " 들머리로 정리한다.
...
- 세균성 외이염 진단, 점이액 처방
[지난 진료 이후]
- 가려움: 지난 진료 대비 호전(3/10)
- 새 증상: 기침 발생, 지난번보다는 줄어듦`;

async function callGeminiForAnalysis(payload: unknown, opts?: { followUp?: boolean }): Promise<GeminiAnalysisResult | null> {
  const apiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;
  if (!apiKey) return null;

  const systemText = opts?.followUp ? `${SYSTEM_PROMPT}\n\n${FOLLOW_UP_SUMMARY_RULE}` : SYSTEM_PROMPT;
  ...
  try {
    ...
    const payload = {
      meta: {
        ...
        // 경과 확인(재진) 방문은 이전 진료 차트를 함께 넘겨 "지난 진료 내용" 소제목 작성에 쓴다.
        previousChartText: session.previousChartText,
      },
    ...
    const analysis = await callGeminiForAnalysis(payload, { followUp: isFollowUpVisit(session.visitType) });
